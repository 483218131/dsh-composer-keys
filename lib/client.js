  ...
  factory: (require) => {
    ...
    var KeyboardShortcuts = function (props) {
      react.useEffect(function () {
        ...
        // True when the caret sits at the very start/end of the input with no
        // selection: "top" = position 0, "bottom" = after the last character.
        // Mid-text Up/Down is never intercepted, so the browser moves the caret
        // and clamps it to the edge first (Home/End style) before cycling starts.
        ...
        function onKeyDown(e) {
          ...
          if (e.key === "ArrowUp") {
            ...
            // Cascade: only take over once the caret already sits at the very
            // start; otherwise let the browser move it up like a plain editor.
            if (!caretAtEdge(active, "top")) return;
            e.preventDefault();
            if (historyIndex === -1) {
            ...
            // Cascade: only take over once the caret already sits at the very
            // end; otherwise let the browser move it down like a plain editor.
            if (!caretAtEdge(active, "bottom")) return;
            e.preventDefault();
