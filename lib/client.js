  ...
  factory: (require) => {
    ...
    var KeyboardShortcuts = function (props) {
      react.useEffect(function () {
        ...
        // True when the caret sits on the given edge of the text with no
        // selection: "top" = on the first line, "bottom" = on the last line.
        function cursorOnEdge(el, edge) {
          var value = el.value || "";
          if (el.selectionStart !== el.selectionEnd) return false;
          if (edge === "top") {
            return value.slice(0, el.selectionStart).indexOf("\n") === -1;
          }
          return value.slice(el.selectionEnd).indexOf("\n") === -1;
        }
        ...
        function onKeyDown(e) {
          ...
          if (e.key === "ArrowUp") {
            ...
            // Cascade: only take over once the caret already sits on the first
            // line; otherwise let the browser move it up like a plain editor.
            if (!cursorOnEdge(active, "top")) return;
            e.preventDefault();
            if (historyIndex === -1) {
            ...
            // Cascade: only take over once the caret already sits on the last
            // line; otherwise let the browser move it down like a plain editor.
            if (!cursorOnEdge(active, "bottom")) return;
            e.preventDefault();
